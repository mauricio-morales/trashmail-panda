...
public interface IEmailArchiveService
{
    ...
    /// <summary>
    /// Executes VACUUM operation to reclaim deleted space.
    /// Expensive operation - acquires exclusive database lock.
    /// </summary>
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>
    /// Success: true if VACUUM completed successfully
    /// Failure: StorageError if database operation fails
    /// </returns>
    Task<Result<bool>> VacuumDatabaseAsync(
        CancellationToken cancellationToken = default);
    ...
    /// <summary>
    /// Gets count of stored feature vectors.
    /// </summary>
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>
    /// Success: Total count of feature vectors
    /// Failure: StorageError if database operation fails
    /// </returns>
    Task<Result<int>> GetFeatureCountAsync(
        CancellationToken cancellationToken = default);
    
    /// <summary>
    /// Gets count of stored email archives.
    /// </summary>
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>
    /// Success: Total count of email archives
    /// Failure: StorageError if database operation fails
    /// </returns>
    Task<Result<int>> GetArchiveCountAsync(
        CancellationToken cancellationToken = default);
    
    /// <summary>
    /// Gets count of user-corrected emails (high retention priority).
    /// </summary>
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>
    /// Success: Count of emails where UserCorrected = true
    /// Failure: StorageError if database operation fails
    /// </returns>
    Task<Result<int>> GetUserCorrectedCountAsync(
        CancellationToken cancellationToken = default);
}
